...
class PathParser {
  ...
  static List<PathDataNode> deepCopyNodes(List<PathDataNode> source) {
    ...
    for (int i = 0; i < source.length; i++)
      copy[i] = PathDataNode.fromOther(source[i]);
    return copy;
  }

  static bool canMorph(
      List<PathDataNode> nodesFrom, List<PathDataNode> nodesTo) {
  ...
  static void updateNodes(
      List<PathDataNode> target, List<PathDataNode> source) {
    for (int i = 0; i < source.length; i++) {
  ...
  static int nextStart(String s, int end) {
    ...
    while (end < s.length) {
      ...
      if ((((code - 'A'.codeUnitAt(0)) * (code - 'Z'.codeUnitAt(0)) <= 0) ||
              ((code - 'a'.codeUnitAt(0)) * (code - 'z'.codeUnitAt(0)) <= 0)) &&
          c != 'e' &&
          c != 'E') {
        return end;
      }
  ...
  static void _addNode(List<PathDataNode> list, String cmd, List<double> val) =>
    ...
    try {
      ...
      while (startPosition < totalLength) {
        ...
        if (startPosition < endPosition)
          results[count++] =
              double.parse(s.substring(startPosition, endPosition));
  ...
  static bool interpolatePathDataNodes(List<PathDataNode> target,
      List<PathDataNode> from, List<PathDataNode> to, double fraction) {
...
class PathDataNode {
  ...
  static void nodesToPath(List<PathDataNode> node, Path path) {
    ...
    for (int i = 0; i < node.length; i++) {
      addCommand(
          path, current, previousCommand, node[i].mType, node[i].mParams);
      previousCommand = node[i].mType;
    }
  }

  void interpolatePathDataNode(
      PathDataNode nodeFrom, PathDataNode nodeTo, double fraction) {
    mType = nodeFrom.mType;
    for (int i = 0; i < nodeFrom.mParams.length; i++)
      mParams[i] =
          nodeFrom.mParams[i] * (1 - fraction) + nodeTo.mParams[i] * fraction;
  }

  static void addCommand(Path path, List<double> current, String previousCmd,
      String cmd, List<double> val) {
    int incr = 2;
    ...
    for (int k = 0; k < val.length; k += incr) {
      switch (cmd) {
        ...
        case 'c': // curveto - Draws a cubic Bézier curve (relative)
          path.relativeCubicTo(val[k + 0], val[k + 1], val[k + 2], val[k + 3],
              val[k + 4], val[k + 5]);
        ...
        case 's': // smooth curveto - Draws a cubic Bézier curve (reflective cp)
          ...
          path.relativeCubicTo(reflectiveCtrlPointX, reflectiveCtrlPointY,
        ...
        case 'S': // shorthand/smooth curveto Draws a cubic Bézier curve(reflective cp)
          ...
          path.cubicTo(reflectiveCtrlPointX, reflectiveCtrlPointY, val[k + 0],
        ...
        case 't': // Draws a quadratic Bézier curve(reflective control point)(relative)
          ...
          path.relativeQuadraticBezierTo(reflectiveCtrlPointX,
              reflectiveCtrlPointY, val[k + 0], val[k + 1]);
        ...
        case 'T': // Draws a quadratic Bézier curve (reflective control point)
          ...
          path.quadraticBezierTo(reflectiveCtrlPointX, reflectiveCtrlPointY,
              val[k + 0], val[k + 1]);
        ...
        case 'a': // Draws an elliptical arc
          ...
          break;
        case 'A': // Draws an elliptical arc
          drawArc(path, currentX, currentY, val[k + 5], val[k + 6], val[k + 0],
  ...
  static void drawArc(
    ...
    if (disc < 0.0) {
      ...
      drawArc(p, x0, y0, x1, y1, a * adjust, b * adjust, theta, isMoreThanHalf,
          isPositiveArc);
  ...
  static void arcToBezier(Path p, double cx, double cy, double a, double b,
    ...
    for (int i = 0; i < numSegments; i++) {
      ...
      double alpha = Math.sin(eta2 - eta1) *
          (Math.sqrt(4 + (3 * tanDiff2 * tanDiff2)) - 1) /
          3;
